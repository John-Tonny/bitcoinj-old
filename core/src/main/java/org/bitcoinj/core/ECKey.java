...
public class ECKey implements EncryptableItem {
     ...
     * @throws SignatureException If the public key could not be recovered or if there was a signature format error.
     */
    public static ECKey signedMessageToKey1(String message, String signatureBase64) throws SignatureException {
        ...
        BigInteger s = new BigInteger(1, Arrays.copyOfRange(signatureEncoded, 33, 65));
        ECDSASignature sig = new ECDSASignature(r, s);
        byte[] messageBytes = Utils.HEX.decode(message);
        // Note that the C++ code doesn't actually seem to specify any character encoding. Presumably it's whatever
        // JSON-SPIRIT hands back. Assume UTF-8 for now.
        Sha256Hash messageHash = Sha256Hash.twiceOf(messageBytes);
        boolean compressed = false;
     ...
     * signature is not the same as this one, throws a SignatureException.
     */
    public void verifyMessage1(String message, String signature) throws SignatureException {
        String signatureBase64 = new String(Base64.encode(Utils.HEX.decode(signature)), StandardCharsets.UTF_8);
        ECKey key = ECKey.signedMessageToKey1(message, signatureBase64);
        if (!key.pub.equals(pub))
            throw new SignatureException("Signature did not match for message");
    }
    ...
    private static final String BITCOIN_SIGNED_MESSAGE_HEADER = "Syscoin Signed Message:\n";
   ...
   /**
     ...
     */
    public Address toAddress(NetworkParameters params, boolean segwitMode) {
        Script.ScriptType outputScriptType = Script.ScriptType.P2PKH;
	if(segwitMode){
	    outputScriptType = Script.ScriptType.P2WPKH;
	}
        return Address.fromKey(params, this, outputScriptType);
    }
